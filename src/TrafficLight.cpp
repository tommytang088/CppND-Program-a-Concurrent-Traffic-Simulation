...
void MessageQueue::send(TrafficLightPhase &&phase) {
...
TrafficLight::TrafficLight() {
  ...
  _cycleRangeLower = 4000;
  _cycleRangeUpper = 6000;
}
...
void TrafficLight::cycleThroughPhases() {
  ...
  std::chrono::time_point<std::chrono::system_clock> cycleEndTime =
      cycleStartTime + std::chrono::milliseconds(calcCycleRange());
  while (true) {
...
int TrafficLight::calcCycleRange() {
  std::random_device rd;
  std::mt19937 eng(rd());
  std::uniform_int_distribution<> distr(_cycleRangeLower, _cycleRangeUpper);
  return distr(eng);
}
